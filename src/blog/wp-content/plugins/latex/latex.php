...
function latex_for_wp_activate() {
        ...
        if (get_option("latex_img_server") == FALSE )
                update_option("latex_img_server", "https://chart.apis.google.com/chart?cht=tx&chl=");
        if (get_option("mathjax_server") == FALSE )     
                update_option("mathjax_server", "https://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML");
...
echo '<script type="text/javascript" src="'.get_option("mathjax_server").'">MathJax.Hub.Queue(["Typeset",MathJax.Hub]);
</script>';
}
}
